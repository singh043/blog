...
const PostPage = () => {
    ...
    return (
        <main className='p-3 flex flex-col max-w-full mx-auto min-h-screen text-black'>
            <div className='max-w-6xl mx-auto'>
                <h1 className="text-3xl mt-10 p-3 text-center font-serif max-w-2xl mx-auto lg:text-4xl">
                    {post && post.title}
                </h1>
                <Link to={`/search?category=${post && post.category}`} className='self-center mt-5'>
                    <button className="py-1 px-2 border-[1px] text-sm rounded-md">{post && post.category}</button>
                </Link>
